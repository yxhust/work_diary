...
## 2020-09-16
1. 迭代优化此前业务看板。
2. [运营思维用于职业规划](http://www.woshipm.com/zhichang/372703.html)
- 跟优秀的人交流，受益匪浅。
- 明白差距，才知道仍需努力。

## 2020-09-15
1. 根据产品内容数据完成重点价值用户看板。
2. 优化以前完成的tableau看板。
...
## 2020-09-02 :cry:
1. 类比RFM模型，在指标划分上出现了难题
- 该业务明细数据包括转化和未转化的，而RFM全部都是购买的用户
- RFM模型中有打分环节，尽量使每个分值间的数据量均衡；打分在本业务指标中不适用，设定打分区间对应的指标无从下手
- RFM模型为何去近度、频度、额度三个维度数据呢？对应的，在本业务中，当前所选的三个维度是否合理呢？未知。

2. 业务临时需求
- 使用决策树训练数据，得到决策树可视化结果
- 根据结果，还原分类标准及过程，给出业务建议


## 2020-09-01 :seedling:
1. 排查产品逻辑问题
- 虽然不影响数据，但排查出的问题确实会影响业务，这种切期操作会丢失原数据
2. 深入学习RFM模型，类比到本业务的用户分层中
- 基于数据集计算维度指标
- 对指标打分
- 后续都是对维度打分做用户价值高低判断
- 用户分层及后续的策略还需结合实际业务来制定
...
## 2020-08-29 & 2020-08-30 :battery:
1. 休息
2. 尝试新事物
3. 坚持锻炼
4. 确定下周重点

## 2020-08-28 :telescope:
1. 新增tableau指标，如消关率、关卡完成率等。
2. 复习区间估计相关知识。
- 95%置信水平下，样本均值+-1.96sigma包含整体均值。
3. 继续优化用户分层明细数据tableau看板。

## 2020-08-27 
1. 结合昨日提取的指标，在tableau尝试有目的的组合，目前完成样例明细数据。
2. 学习实用tableau技巧。
- 技巧记录在有道云笔记，包括度量名称与度量值，环比的颜色呈现方式，饼图+文字实现方法
- 难点是：表计算，窗口函数。

## 2020-08-26 :turtle:
1. 完成用户生命周期的数据提取，只取真正对业务有帮助的指标，仅通过SQL就可以。
- 原先有一个指标是“连续使用产品天数”，因为用户不连续使用也存在转化的可能。且我们的产品运营周期较短。
- 相对原先指标，另一个指标“每路径距开始的时间差”能更好的替代，符合运营节奏，有利于运营同事把握用户的积极性。
2. 迭代产品最小粒度--资源的tableau仪表盘。
- 布局重大调整，调整布局后的仪表盘很不错。
- SQL逻辑上调整，精简数据量的同时能完成产品资源数据监控情况。
3. 继续学习实用的tableau技巧。
- 容器的使用
- 参数的滑块显示
- 工作表用作筛选器
